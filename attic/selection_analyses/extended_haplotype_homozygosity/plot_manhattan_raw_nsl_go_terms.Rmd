...
raw_nsl_values_df <- raw_nsl_values[,c(1,2,9)]
raw_nsl_values_df$SNP<- rownames(raw_nsl_values_df)
raw_nsl_values_df$V9 <- abs(raw_nsl_values_df$V9)
...
chrom_order <- as.character(sort(unique(as.numeric(axisdf$CHR))))

```


```{r}
manhattan_raw_plot<- ggplot(input_for_manhattan_rw, aes(x=BPcum, y=P)) +
            ...
            geom_hline(yintercept = c(2.56), linetype="dashed", colour="black") +
            xlab("Chromosome") +
            ylab("-log10(P)") +
            # Custom the theme:
            theme_bw() +
...
manhattan_raw_plot <- manhattan_raw_plot +         theme(axis.text=element_text(size=15),
