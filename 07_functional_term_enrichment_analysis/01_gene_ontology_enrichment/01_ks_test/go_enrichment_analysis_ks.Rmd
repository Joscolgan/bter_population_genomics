...
nsl_ranked_file <- "input/top_nsl_for_all_genes_plus_corrected_length_ensembl.txt"
...
nsl_ranked_sorted$length_corrected <- 1 - (nsl_ranked_sorted$length / max(nsl_ranked_sorted$length))

## Read in GO annotations: 
gene_to_go_mapping <- readMappings(file = gene_to_go_mapping_file)

## Convert into topgo's genelist format:
topgo_genelist        <- nsl_ranked_sorted$length_corrected
...
```{r, message = FALSE}
## Check style:
lintr::lint(file = "./go_enrichment_analysis_ks.Rmd")
```
