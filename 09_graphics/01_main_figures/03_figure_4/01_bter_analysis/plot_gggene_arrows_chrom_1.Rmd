---
title: "Bombus population genomics"
output: plot_gggenes_arrows_chrom_1.html
author: Joe Colgan, Yannick Wurm http://wurmlab.com
---

## Introduction
This script takes a tab-delimited file as input containing the following columns:
1) Chromosome name
2) Start position of chromosome
3) End position of chromosome
4) Gene name
5) Strand
6) Status - if you want to highlight a gene of interest

1. Load libraries:

```{r, message = FALSE}
## Load libraries:
libraries <- c("ggplot2",
               "ggpubr",
               "gggenes",
               "lintr")
for (lib in libraries) {
        if (require(package = lib, character.only = TRUE)) {
                print("Successful")
        } else {
                print("Installing")
                source("https://bioconductor.org/biocLite.R")
                biocLite(pkgs = lib)
                library(lib, character.only = TRUE)
        }
}
```

2. Read input:

```{r, message = FALSE}
## Read in file for plotting:
input <- "input/gggenes_input/chromosome_one_input_for_gggenes.txt"
data  <- read.table(file = input,
                    header = TRUE)

## Assign values to start and end positions:
start_position <- 362267 - 100000
end_position   <- 560366 + 100000

## Turn off scientific notation:
options(scipen = 999)

data$direction <- ifelse(data$strand == "forward", 1, -1)

## Plot:
arrow_plot <- ggplot(data,
                     aes(xmin = as.numeric(start),
                         xmax = as.numeric(end),
                         y = strand,
                         fill = status,
                         #label = gene,
                         forward = direction)) +
        xlim(start_position, end_position) +
        geom_gene_arrow(arrowhead_height = unit(3, "mm"),
                        arrowhead_width = unit(1, "mm")) +
        facet_wrap(~ molecule,
                   scales = "free",
                   nrow = 1,
                   shrink = FALSE) +
        scale_fill_manual(values = c("Beige",
                                     "Grey75")) +
        theme_genes() +
        theme(axis.title.y = element_blank(),
              axis.text.y = element_blank(),
              axis.title.x = element_blank(),
              axis.text.x = element_blank(),
              axis.line.x = element_blank(),
              axis.ticks.x = element_blank(),
              legend.position = "none")

## Combine plot:
save(arrow_plot,
     file = "results/chrom_1_arrow_plot.Rdata")
```

3. Run lintr to check style-guide:

```{r, message = FALSE}
## Run lintr:
lintr::lint(file = "plot_gggene_arrows_chrom_1.Rmd")
```
