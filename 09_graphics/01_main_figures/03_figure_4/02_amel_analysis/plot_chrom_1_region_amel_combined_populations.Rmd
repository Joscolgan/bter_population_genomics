...
output: plot_chrom_1_region_amel_combined_populations.html
...
2. Read in input data containing nucleotide diversity and Tajima's D estimates for four populations:  

```{r, message = FALSE}
## Both nucleotide diversity and Tajima D are in the same input file:
input_path <- "input/nuc_div/"
...
5. Generate combined scatterplots:
...
7. Generate combined multi-image plots:

```{r, message=FALSE}
## Generate a combined plot:
arrow_plot_one <- arrow_plot_one + theme(legend.position = "none")
arrow_plot_two <- arrow_plot_two + theme(legend.position = "none")
region_two_plot <- region_two_plot + theme(legend.position = "none")
...
```{r, message = FALSE}
## Run lintr:
lintr::lint(file = "plot_chrom_1_region_amel_combined_populations.Rmd")
```
