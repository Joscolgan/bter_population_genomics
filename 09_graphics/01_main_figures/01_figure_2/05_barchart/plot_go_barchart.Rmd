...
## Introduction:  
This script is for visualisation of the output of gene ontology (GO) enrichment analyses for raw |nsl| scores performed by [topGo](https://www.bioconductor.org/packages/release/bioc/html/topGO.html).  
...
This script outputs a barchart as a saved R object (.RData) or graphic (.png, .pdf).  
...
combined_terms_plot <- combined_terms_plot +
        labs(x = expression(bold("Gene Ontology terms enriched for genes with strongest |"~italic(n)*S[L]*"| scores"))) +
...
combined_terms_plot <-  combined_terms_plot +
        annotate("text",
                 ...
                 colour = "royalblue3") +
        annotate("text",
                 x = nrow(bp_terms) + (nrow(cc_terms) / 2),
                 y = 21,
                 label = c("Cellular component"),
                 fontface = 2,
                 size = 5,
                 colour = "goldenrod3") +
        annotate("text",
