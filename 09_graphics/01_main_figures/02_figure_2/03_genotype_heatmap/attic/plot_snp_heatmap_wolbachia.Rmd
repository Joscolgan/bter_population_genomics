...
2. Load in VCF containing genotypes of interest:
...
3. Plot heatmap to demonstrate representation of haplotypes within population:
...
## Check if site names are the same - if so, add different subsite letters:
...
## Update rownames with new site names:
...
dat <- input_vcf_gds_genofile_matrix[, c(100:200)]

## Perform hierarchical clustering:
row_order <- hclust(dist(dat))$order
col_order <- hclust(dist(t(input_vcf_gds_genofile_matrix_subset_df)))$order

## re-order matrix accoring to clustering
dat_new <- input_vcf_gds_genofile_matrix_subset_df[row_order, ]
