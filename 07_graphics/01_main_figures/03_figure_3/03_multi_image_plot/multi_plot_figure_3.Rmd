...
output: multi_plot_figure_3.html
...
## Load arrow plot:
load(file = "results/plots/chrom_1_genotype_heatmap.RData")
...
region_histo_plot <- ggplot(data = nuc_div_data_100kb) +
        geom_histogram(aes(x = seg_sites,
                           fill = is_low),
                       bins = 100) +
        xlab("Number of SNPs per 100 kb sliding window") +
        ylab("Count") +
        theme_bw() +
...
```{r, message=FALSE}
## Generate a combined plot for bumblebee plots:
...
## Save as PDF:
ggsave(file = "results/figure_3_combined.pdf",
       dpi = 600,
       width = 13,
       height = 14)

## Save as PNG:
ggsave(file = "results/figure_3_combined.png",
       width = 13,
       height = 14)
...
```{r, message = FALSE}
lintr::lint(file = "multi_plot_figure_3.Rmd")
```