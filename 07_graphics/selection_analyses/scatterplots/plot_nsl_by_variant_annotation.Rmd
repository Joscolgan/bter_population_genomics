...
data_sig <- subset(data, nsl_score > 2.56)
        
updwn_stream_sig_df <- subset(data_sig,
                              rownames(data_sig) %in%
                                      updwn_stream_variants)

intergenic_sig_df <- subset(data_sig,
                            rownames(data_sig) %in% intergenic_variants)

genic_sig_df <- subset(data_sig,
                       !(rownames(data_sig) %in% non_genic))

intronic_sig_df <- subset(data_sig,
                          rownames(data_sig) %in% intronic_variants)

utr_sig_df <- subset(data_sig,
                     rownames(data_sig) %in% utr_variants)

splice_sig_df <- subset(data_sig,
                        rownames(data_sig) %in% splice_variants)

genic_sig_df <- subset(data_sig,
                       !(rownames(data_sig) %in% non_coding))

## Generate plot:
variant_sig_histogram <- ggplot(data = data_sig,
                            aes(x = nsl_score)) +
        geom_histogram(data = intronic_sig_df,
                       aes(fill = "intronic"),
                       colour = "black") +
        geom_histogram(data = updwn_stream_sig_df,
                       aes(fill = "5Kb up/downstream"),
                       colour = "black") +
        geom_histogram(data = intergenic_sig_df,
                       aes(fill = "intergenic"),
                       colour = "black") +
        geom_histogram(data = utr_sig_df,
                       aes(fill = "UTR"),
                       colour = "black") +
        geom_histogram(data = genic_sig_df,
                       aes(fill = "coding"),
                       colour = "black") +
        geom_histogram(data = splice_sig_df,
                       aes(fill = "splice variants"),
                       colour = "black") +
        theme_bw() +
        ylab("Number of SNPs") +
...
ggarrange(variant_histogram,
          variant_sig_histogram,
          labels = c("A", "B"),
          ncol = 2,
          nrow = 1,
          common.legend = TRUE)

ggsave(file = "./combined_plot_test.png",
       dpi = 600,
       height = 10,
       width = 10)
```
...
Read in Manhattan plot to generate a multiple image plot:
```{r, message = FALSE}
## Load manhattan plot:
