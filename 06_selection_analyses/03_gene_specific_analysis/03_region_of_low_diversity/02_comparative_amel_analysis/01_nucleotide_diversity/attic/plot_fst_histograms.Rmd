...
```{r, message = FALSE}
input <- read.table(file = "./african_vs_wEurope.txt.weir.fst",
                    header = TRUE)

input_complete <- input[!is.na(input$WEIR_AND_COCKERHAM_FST), ]

median_fst <- median(input_complete$WEIR_AND_COCKERHAM_FST)

test <- ggplot(data = input_complete,
       aes(x = WEIR_AND_COCKERHAM_FST)) +
        geom_histogram()
