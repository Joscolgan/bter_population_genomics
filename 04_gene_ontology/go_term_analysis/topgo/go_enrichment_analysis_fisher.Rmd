...
## Step One:
## Define input:
input <- "./input/dmel_bter_go_term_database.txt"

## GO annotations
gene_to_go_mapping_file <- input

## genes overlapping with regions of significantly low nucleotide diversity:
gene_list <- scan(file = "input/zscore_normalised_sig_low_div_tjd_merged_flanking_regions_overlap_gene_ids.txt",
                  as.character())

## Define node size:
node_size <- 50

output_directory <- paste("results/go_term_output_",
                          node_size,
                          "_fisher",
                          sep = "")
...
genesOfInterest <- as.character(gene_list)
