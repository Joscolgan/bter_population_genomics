...
dir.create("results/figures/median",
           recursive = TRUE)
```
...
bp_terms <- read.table(file = "results/median/output_50_ks/BP_sig.tsv",
                       header = TRUE)
bp_terms$category <- "BP"

## Read in molecular function GO terms:
mf_terms <- read.table(file = "results/median/output_50_ks/MF_sig.tsv",
                       header = TRUE)
mf_terms$category <- "MF"

## Read in molecular function GO terms:
cc_terms <- read.table(file = "results/median/output_50_ks/CC_sig.tsv",
                       header = TRUE)
...
## Subset with a more conservative pvalue for plotting:
combined_terms <- subset(combined_terms,
                         weight_ks < 0.04)

## Log transform p values for plotting:
combined_terms$weight_ks <- -log(combined_terms$weight_ks)
...
## Save picture:
ggsave(file = "results/figures/median/combined_go_terms_barchart.png",
       height = 12,
       width = 14)
```
