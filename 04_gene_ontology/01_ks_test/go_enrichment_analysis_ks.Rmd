...
## Step One:
## Define input:
input <- "./input/database/dmel_bter_go_term_database.txt"

## GO annotations
gene_to_go_mapping_file <- input

## file of significant genes (2 column file: i.e. gene id and pvalue) file:
nsl_ranked_file <- "./input/nsl/top_nsl_for_all_genes_plus_length.txt"
...
one_percent <- subset(nsl_ranked, nsl_score > quantile(nsl_ranked$nsl_score, 0.99))
five_percent <- subset(nsl_ranked, nsl_score > quantile(nsl_ranked$nsl_score, 0.95))
