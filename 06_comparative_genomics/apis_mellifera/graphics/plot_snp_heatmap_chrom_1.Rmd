...
```{r, message = FALSE}
## Define input:
input_path <- "./input_for_plot/vcf/"
input_vcf <- paste(input_path,
                   "region_1_snp_positions.txt.recode.vcf",
                   sep = "")
...
## Plot:
heatmap_plot_one <- ggplot(dat_new_melted_ordered,
...
## Output plot:
ggsave(file = "./results/figures/snp_heatmap_region_1.png",
       height = 10,
       width = 10)
```
...
input_path <- "./input_for_plot/vcf/"
input_vcf <- paste(input_path,
                   "region_2_snp_positions.txt.recode.vcf",
                   sep = "")
## Use the name of input to designate the name for the GDS file format.
input_vcf_gds <- gsub("recode.vcf",
                      "recode.gds",
...
## Convert into dataframe:
input_vcf_gds_genofile_matrix_subset_df <- as.data.frame(input_vcf_gds_genofile_matrix)
dat <- input_vcf_gds_genofile_matrix[, c(1500:2500)]
row_order <- hclust(dist(dat))$order # clustering
col_order <- hclust(dist(t(dat)))$order
dat_new <- dat[row_order, col_order] # re-order matrix accoring to clustering
dat_new_df <- as.data.frame(dat_new)
...
## Plot:
heatmap_plot_two <- ggplot(dat_new_melted_ordered,
                       aes(variable, samples)) +
                  geom_tile(aes(fill = value), colour = "white") +
...
ggsave(file = "./results/figures/snp_heatmap_region_2.png",
       height = 10,
       width = 10)
```
