...
bin_width <- 10000
input_gff <- "../GCF_000002195.4_Amel_4.5_genomic.gff"
output <- "apis_all_chrom_10kb.txt"
...
for (item in 1:nrow(chrom_data)){
        ...
        ## Read in VCF file:
        chrom_data_item <- readVCF(chrom_data[item,][1], bin_width, chrom, chrom_data[item,][2], chrom_data[item,][3], gffpath = input_gff, approx = F,  include.unknown=TRUE)
        ##
