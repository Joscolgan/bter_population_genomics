...
## Introduction:
This script reads in a VCF file and generates a genotype matrix using [SNPRelate](https://bioconductor.org/packages/release/bioc/html/SNPRelate.html).
...
Load input vcfs for generating genotype matrices:

```{r, message = FALSE}
## Define input:
input_path <- "./filtered_vcf/"
input_vcf <- paste(input_path,
                   "amel_region_1.recode.vcf",
                   sep = "")
...
positions <- read.table(paste(input_path,
                              "amel_region_1_snp_positions.txt",
                              sep = ""),
                        header = FALSE)
