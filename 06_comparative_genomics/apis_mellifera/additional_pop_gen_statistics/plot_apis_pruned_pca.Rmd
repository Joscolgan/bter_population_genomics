...
## Introduction:
This script takes a VCF as input, converts to a genotype matrix using [SNPRelate](https://www.bioconductor.org/packages/release/bioc/html/SNPRelate.html), prunes based on LD then performs a principal component analysis. 
The script also generates a scatterplot of user-defined principal components using [ggplot2](https://ggplot2.tidyverse.org/)
The script outputs an image file (.png, .jpeg, .pdf) of PCA scatterplot.

```{r, message = TRUE}
## Plot PCA of larger number of samples:
...
## In this format, each byte encodes up to four SNP genotypes
## thereby reducing file size and access time.
## The GDS format supports data blocking so that only the subset
## of data that is being processed needs
## to reside in memory. GDS formatted data is also designed for
## efficient data access to large datasets.

## Use the name of input to designate the name for the GDS file format.
input_apis_vcf_gds <- gsub(".vcf",
                           ".gds",
                           input_apis_vcf)
...
## This file should contain 'treatment information' for each
## sample and be in the same order as each sample.
## i.e. the first treatment information would be assigned to
## the first sample, etc., etc.
## Read in table:
...
## Assign site number for plotting:
...
Check code adheres to recommended style guide using lintr:

```{r, message = FALSE}
lintr::lint(file = "./plot_apis_pruned_pca.Rmd")
```