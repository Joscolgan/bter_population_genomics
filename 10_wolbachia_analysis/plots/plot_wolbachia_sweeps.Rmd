...
## Introduction:  
This script takes numerous tab-delimited text files and R objects as input to generate a multiple image plot
generated through a combination of [ggplot2](https://ggplot2.tidyverse.org/) and [ggpubr](http://www.sthda.com/english/articles/24-ggpubr-publication-ready-plots/).  
For input, this script specifically takes the following:  
1) A tab-delimited text file containing five columns:  
a) chrom: Chromosome where a polymorphic site (SNP) is located.  
b) position: The phyiscal position of the SNP of interest.  
c) nsl_score: A nsl score assigned to the SNP by [selscan](https://github.com/szpiech/selscan).  
d) annotation: Functional annotation of the SNP assigned by [SnpEff](http://snpeff.sourceforge.net/SnpEff_manual.html).  
e) locus: The gene symbol where the SNP is located or at least within 5kb.  
