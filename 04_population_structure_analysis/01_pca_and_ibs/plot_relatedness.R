...
##
## Purpose:  
## This script takes a VCF file as input (VCF generated by Freebayes) and 
## using gdsfmt generates a genotype matrix.
## Using this genotypic matrix, SNPRelate performs LD pruning, PCA and isolation
## by descent to assess relatedness amongst individuals.
...
## Extract identifiers for pruned SNPs:
snplist <- snpgdsSNPList(input_vcf_gds_genofile)

## Subsey positions of subsetted unlinked SNPs:
snpset_df <- subset(snplist, snp.id %in% snpset.id)

## Write dataframe containing subsetted SNP lcoations to file:
