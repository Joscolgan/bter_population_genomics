...
```{r, message = FALSE}
## Read in input file:
data <- read.table(file = "./input/nucleotide_diversity_output/bter_all_chrom_100kb.txt",
                   header = TRUE)

## Calculate z-score for nucleotide diversity:
data_nd_sd <- sd(data$nuc_diversity)*sqrt((length(data$nuc_diversity)-1)/(length(data$nuc_diversity)))
...
data_nd_sig <- subset(data, nd_pvalue2sided < 0.05)
data_td_sig <- subset(data, td_pvalue2sided < 0.05)
