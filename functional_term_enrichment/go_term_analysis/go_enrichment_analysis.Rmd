...
## Step One
## GO annotations
gene_to_go_mapping_file <- "input/dmel_vs_bter_biomart.input_for_converter.output.txt"

## file of significant genes (2 column file: i.e. gene id and nsl value) file:
nsl_genes_file <- "input/2018-02-08_tmp_file_filtered_sorted-onlyscored.txt"

## Read table in:
nsl_ranked <- read.table(file=nsl_genes_file, header = FALSE)

## Update column names:
colnames(nsl_ranked) = c("locus", "nsl_score")
topgo_genelist <-nsl_ranked$nsl_score
names(topgo_genelist) <- nsl_ranked$locus

# now we mess around to make the nSL ranking go from strongest to weakest simialrly to 
# what we see with pvalues. 
topgo_genelist =  1 - topgo_genelist/max(topgo_genelist)

## Create an output directory:
output_directory <- "test_new_output"
...
for (go_category in c("BP", "MF", "CC")) {
  ...
  result_weight_output$weight_ks_adjusted <- p.adjust(p=result_weight_output$weight_ks, method = c("bonferroni"))
            
  ## Subset calls with significance higher than expected:
  result_weight_output_sig <- subset(x      = result_weight_output,
                                     subset = (Significant > Expected) & (weight_ks_adjusted < 0.05))
    ## Write to output:
        write.table(x         = result_weight_output,
              file      = file.path(output_directory,
                                    paste(go_category, "all.tsv", sep = "_")),
              row.names = FALSE,
              sep       = "\t",
              quote = FALSE)
...
Code for examining GO term plots:
